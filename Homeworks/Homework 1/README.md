...
Simulate the circuit fed with a resistive load of R = 75 Ω. Ignore any non-idealities and take AC source as Turkish electricity grid phase-neutral voltage.
...
**b)** Using simulation tools, find the minimum output filter capacitance to have 5% output voltage peak-to-peak ripple to mean ratio. Use the values of same AC voltage source in part 1, L<sub>S</sub> = 4 mH, R<sub>S</sub> = 1.8 mΩ and R<sub>LOAD</sub> = 60 Ω. Assume diodes are ideal.
...
**a)** Simulate the circuit for L<sub>s</sub>=0, V<sub>an</sub>, V<sub>bn</sub> and V<sub>cn</sub> are balanced three phase voltages connected to the same grid as before, diodes are ideal and I<sub>d</sub>=20 A. Plot the output voltage and phase A input current on the same graph.
...
**d)** Simulate the same rectifier for L<sub>s</sub>=0.5 mH and plot the output voltage and input current. Observe the effect of line inductance and comment on your observations.

**e)** Repeat part (b) with L<sub>s</sub>=0.5mH.

**f)** Compare the harmonic content of the input current for different L<sub>s</sub> values of 0, 0.5 mH, 2.5 mH and 25 mH by checking IEEE 519-2014 standard. Comment on the differences. What kind of problems can having high input current THD cause to the grid?
...
* The load current has a step change from 20 A to 10 A with line inductances (0.5 mH) and without line inductances.

* The load current has a step change from 10 A to 20 A with line inductances (0.5 mH) and without line inductances.
